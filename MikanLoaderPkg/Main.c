...
{
    Elf64_Phdr* phdr = (Elf64_Phdr*)((UINT64)ehdr + ehdr->e_phoff); // e_phoffはプログラムヘッダーのオフセットを表す項目
    *first = MAX_UINT64;
    *last = 0;
    for (Elf64_Half i = 0; i < ehdr->e_phnum; i++) { // e_phnumは要素数を表す項目
...
void CopyLoadSegments(Elf64_Ehdr* ehdr) // ehdr: コピー元のメモリの先頭アドレス
{
...
EFI_STATUS EFIAPI UefiMain(
    ...
    // カーネルファイルを一時領域に読み込む. kernel_bufferはその領域の先頭アドレス.
    VOID* kernel_buffer;
